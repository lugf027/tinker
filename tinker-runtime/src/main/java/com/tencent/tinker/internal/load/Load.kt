...
private fun Application.loadWith(
    ...
    val factories = buildList {
        if (!hardening && Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            traceS("load.createFactory(type = code#v31_non_hardening)") {
            ...
            traceS("load.createFactory(type = code#v27_non_hardening)") {
            ...
            traceS("load.createFactory(type = code#v24_non_hardening)") {
            ...
            traceS("load.createFactory(type = code#inject_path)") {
        ...
        traceS("load.createFactory(type = resource)") {
